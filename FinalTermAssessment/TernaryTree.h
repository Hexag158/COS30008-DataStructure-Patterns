...
public:
    ...
    {
        if (empty()) throw domain_error("The tree is empty");
        return fKey;
    }
    ...
    size_t height() const
    {
        ...
        size_t lHeight[3] = {};
        for (int i = 0; i < 3; i++) 
            lHeight[i] = fSubTrees[i]->empty() ? 0:fSubTrees[i]->height();

        return *max_element(lHeight, lHeight + 3) + 1;
    }
...
// Problem 2: TernaryTree Copy Semantics
    ...
    {
        if (empty()) throw domain_error("Cannot copy empty tree");
...
// Problem 3: TernaryTree Move Semantics
	...
	TTree& operator=( TTree&& aOtherTTree )
    {
        if (this != &aOtherTTree)
            ...
            { 
                throw domain_error("Cannot move NIL"); 
            }
    }

...
int main( int argc, char* argv[] )
{
    ...
    if (argc < 2)
    {
        cerr << "Arguments missing." << endl; //cerr = character error
        cerr << "Usage: VectorOperations <filename>" << endl; //cerr = character error
    ...
    Polygon lPolygon; //define the polygon
    
    lPolygon.readData( lInput ); //read data from the input file
    
    lInput.close(); //close input file after read

    cout << "Data read:" << endl; 
    
    for ( size_t i = 0; i < lPolygon.getNumberOfVertices(); i++ ) //for loop to loop through the data
    {
        cout << "Vertex #" << i << ": " << lPolygon.getVertex( i ) << endl; // print out each vertex fom the input file
    }

    runProblem1( lPolygon ); //call the function to run problem 1 solution with Polygon 
#endif
 
#ifdef P2
    runProblem2();//call the function to run problem 2 solution no parameter
#endif
 
#ifdef P3
    runProblem3(); //call the function to run problem 3 solution
#endif

#ifdef P4
    runProblem4(); //call the function to run problem 4 solution
#endif
	return 0;
}